...
const COMBO_TIERS = [
  { min: 7, gradient: 'from-red-500 via-orange-500 to-yellow-400', glow: 'rgba(251,191,36,0.5)', scale: 1.5, label: '超神连击！' },
  { min: 5, gradient: 'from-orange-500 via-amber-500 to-yellow-400', glow: 'rgba(249,115,22,0.4)', scale: 1.35, label: '势不可挡！' },
  { min: 3, gradient: 'from-yellow-400 to-amber-500', glow: 'rgba(234,179,8,0.35)', scale: 1.2, label: '' },
]
...
export default function ComboIndicator({ combo, prevCombo }) {
  ...
  return (
    <div className="fixed top-1/3 left-1/2 -translate-x-1/2 z-30 pointer-events-none" role="status" aria-live="polite" aria-label={`${displayCombo}连击`}>
      <div
        className={`combo-pop combo-glow bg-gradient-to-r ${tier.gradient} text-white
