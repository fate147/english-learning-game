...
export default function ErrorRanking({ errorRanking, subject = 'english' }) {
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-700/35 rounded-xl px-5 py-4">
      <h4 className="section-title text-slate-200"><span>{TITLE_MAP[subject] || '🔴 常错记录'}</span> Top 5</h4>
      <div className="flex flex-col gap-1.5">
        {errorRanking.map(([key, count], index) => {
          const displayName = subject === 'english' ? (getWordById(key)?.word || key) : key
          const badgeClass = count >= 5 ? 'bg-red-500/15 text-red-400' : 'bg-yellow-500/15 text-yellow-400'
          return (
            <div key={key} className="flex items-center gap-2 px-2.5 py-2 rounded-lg bg-slate-900/30">
              ...
              <span className="flex-1 font-semibold text-sm text-slate-200">{displayName}</span>
